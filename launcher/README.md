...
- **Настройки:** RAM, Java, папка инстанса, разрешение, полноэкранный режим,
  офлайн-профиль и действие после запуска игры: оставить, свернуть или закрыть
  лаунчер.
...
- Лог запуска сохраняется в `laura-launcher.log` внутри папки инстанса.
  Файл перезаписывается при каждом запуске (как `latest.log` у Minecraft), поэтому
  в нём всегда только последний запуск — старый краш не выглядит как текущий.
