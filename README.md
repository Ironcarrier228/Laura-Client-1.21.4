...
- 🛠️ **127 модулей** в 6 категориях (Combat, Movement, Render, Player, Misc, Bots) с настройками.
...
- **Bots**: AutoPottBot (авто-варка зелий), AutoAncientBot (авто-фарм обломков через ТНТ)…
...
127 модулей, сгруппированных по категориям:

| Категория | Описание | Кол-во |
|---|---|---|
| ⚔️ **Combat** | Атака, прицел, защита, анти-кик | 23 |
| 🏃 **Movement** | Полёт, движение, элитра, камера | 22 |
| ✨ **Render** | Видение, ESP, шейдеры, интерфейс, визуальные эффекты | 31 |
| 🧍 **Player** | Автоматизация, инвентарь, утилиты | 27 |
| 🛠️ **Misc** | Фарм, серверные помощники, чат, звук | 22 |
| 🤖 **Bots** | Автономные боты: варка зелий, фарм обломков | 2 |
| **Итого** | | **127** |

### 🔁 Портировано из Wexside

Часть модулей перенесена из клиента [Wexside 1.21.8](https://github.com/pidid64-alt/Wexside-1.21.8)
и адаптирована под архитектуру Laura Client (1.21.4):

| Модуль | Категория | Что делает |
|---|---|---|
| **Speed** | Movement | Ускоряет персонажа (режимы «Ванильный» / «Буст»). |
| **NoFall** | Movement | Предотвращает урон от падения (режимы «Пакет» / «Элитры»). |
| **Timer** | Movement | Ускоряет игру (режимы «Умный» / «Бёрст» / «Грим»). |
| **Jesus** | Movement | Позволяет ходить по воде и лаве. |
| **NoWeb** | Movement | Убирает замедление в паутине. |
| **Criticals** | Combat | Критический удар в паутине или под плавным падением. |
| **FastBow** | Combat | Быстрая стрельба из лука без полного натяжения. |
| **BowHelper** | Combat | Плавная баллистическая наводка лука/арбалета на противника. |
| **FakePlayer** | Player | Локальный бот для тренировки атак и тотемов. |
| **AutoInvisible** | Player | Автоматически выпивает зелье невидимости. |
| **AutoDrop** | Misc | Автоматически выбрасывает выбранный мусор из инвентаря. |
| **AutoPottBot** | Bots | Авто-варка зелий: обслуживает варочные стойки, наполняет бутылки водой, складывает готовые зелья в сундук. |
| **AutoAncientBot** | Bots | Авто-фарм древних обломков через ТНТ: выбор площадки, установка и поджиг ТНТ, добыча обломков, пёрки и побег из лавы. |
| **AncientXray** | Render | Подсвечивает древние обломки, обнажённые взрывом ТНТ (сканирование по пакетам взрывов). |
...
Виджет «Сейчас играет» (категория **Render**, модуль **Music HUD**) показывает
обложку, название трека, исполнителя, прогресс-бар и позволяет управлять плеером
горячими клавишами. Работает через бесплатные провайдеры: **YouTube Music**, **wolfXspotify**, **SoundCloud** и опционально **Spotify** (по API).

### Бесплатные провайдеры

| Провайдер | Авторизация | Описание |
|---|---|---|
| **YouTube Music** | Не требуется | Встроенный поиск через InnerTube API. Введите название трека в настройках. |
| **wolfXspotify** | Не требуется | Прокси-сервер, не требующий ключей. Просто выберите источник. |
| **SoundCloud** | Не требуется | Бесплатный провайдер музыки. |

### Опциональный Spotify

Spotify поддерживается как опциональный источник. Для него требуются **Client ID**, **Client Secret** и **Refresh Token** из [Spotify Developer Dashboard](https://developer.spotify.com/dashboard).

### Настройка (1 раз)

1. Выберите источник музыки в настройках модуля.
2. Для **YouTube Music** — введите название трека для поиска.
3. Для **Spotify** — получите Client ID, Client Secret и Refresh Token (см. выше).
...
- `YouTube Music не найден` / `SoundCloud не подключён` — введите поисковый запрос или выберите другой источник.
- `Ошибка авторизации` — данные Spotify не приняты (показан HTTP-код причины).
- `Нет соединения` — нет доступа к интернету/API.
- `Ничего не играет` — источник работает, но сейчас ничего воспроизводится.
