...
public class WayCommand extends BaseCommand {
                ...
                if (message.contains("[1] Маяк убийца") || message.contains("[1] Вулкан") || message.contains("[1] Метеоритный дождь") || message.contains("[1] Гейзер")) {
                    for (int j = i; j < this.d.size(); j++) {
                        ...
                        if (next.contains("|| Координаты:")) {
                            ...
                            if (this.e == a.GPS) {
                                Laura.getInstance().getModuleProcessor().u().d().a(pos);
                            } else {
                                a("Ивент", pos);
                            }
                            found = true;
    ...
    public void a(DrawEvent event) {
        if (event.b()) {
            ...
            Vec3d gps = Laura.getInstance().getModuleProcessor().u().d().c();
            if (gps != null) {
    ...
    private void a(DrawEvent event, b way, Vec3d eyes) {
        ...
        if (ProjectUtil.isOnScreen(screen)) {
            ...
            int background = Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.BACKGROUND_HUD).toIntColor();
            ...
            event.getDraw2DProcessor().a(event.h(), x, y, width, 11.5f, 3.5f, ColorUtil.applyAlphaToColor(background, Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.BACKGROUND_HUD).getAlphaFloat()), 1.0f, ColorUtil.applyAlphaToColor(background, Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.BACKGROUND_HUD).getAlphaFloat()), 6.0f);
    ...
    private void a(DrawEvent event, Vec3d gps, Vec3d eyes) {
        ...
        event.getDraw2DProcessor().a(matrices, Identifier.of("laura", "pictures/triangle.png"), -7.0f, -7.0f, 14.0f, 14.0f, 0.0f, primary);
        matrices.pop();
