...
import laura.module.misc.Collector;
import laura.render.*;
import laura.ui.element.Section;
import laura.util.Marker;
import laura.util.MathUtil;
import laura.util.ProcessIdUtil;
...
public class CollectorSection extends Section {
    ...
    private void drawItemGrid(DrawContext context, Vector4f frame, float slot, float gap, int columns, float top, float scroll, int mouseX, int mouseY, float delta) {
        ...
        List<Collector.b> items = Laura.getInstance().getModuleProcessor().p().e();
        this.g = 0.0f;
        this.d = null;
