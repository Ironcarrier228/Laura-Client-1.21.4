...
public class Communication extends Module {
    ...
    public Communication() {
        BindSetting b = new BindSetting("Отправление метки друзьям", -1).a(() -> {
            ...
            Laura.getInstance().f().a(false, "friend", "type", "mark", "pos", posObject);
        });
        a(b, this.c);
    }
        ...
        if (this.c.c().booleanValue() && event.isSend()) {
            ...
            if (packet instanceof ChatMessageC2SPacket) {
                ...
                if (content.startsWith("@")) {
                    Laura.getInstance().f().a(false, "irc", "message", content.substring(1));
                    event.a(true);
                }
            }
        }
    ...
    public void onBackend(BackendEvent event) {
        ...
        if ("irc".equals(packet.getId()) && this.c.c().booleanValue()) {
            ...
            if (prefix != null) {
                line.append(Text.literal(prefix.a()).setStyle(Style.EMPTY.withFont(Identifier.of("laura", "prefixes")))).append(Text.literal(StringUtils.a));
            }
