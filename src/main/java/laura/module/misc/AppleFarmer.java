...
public class AppleFarmer extends Module {
    ...
    public void a(TickEvent event) {
        ...
        Laura.getInstance().getModuleProcessor().t().aV().b(19);
        if (Laura.getInstance().getModuleProcessor().v().getInteractHandler().hasTasks() || !Laura.getInstance().getModuleProcessor().v().getPvEHandler().startMendingTask(mc.player.getMainHandStack(), 10.0d, 98.0d)) {
            return;
        }
    ...
    private void a(Predicate<ItemStack> match) {
        ...
        Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(slot, target, 1);
    ...
    private void a(Rotation rotation) {
        Laura.getInstance().getModuleProcessor().k().startAiming(rotation, 180.0f, 1, 1);
    }
