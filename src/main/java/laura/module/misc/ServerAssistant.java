...
import laura.config.DescriptionProcessor;
import laura.config.EnchantmentProcessor;
import laura.core.*;
import laura.core.Module;
import laura.event.ContainerEvent;
import laura.event.PacketEvent;
import laura.event.TickEvent;
import laura.event.TooltipEvent;
        ...
        if (this.autoGodAura.c().booleanValue()) {
            if (ServerUtil.a.a$() || ServerUtil.d.a()) {
                ...
                if (InventoryUtil.b(Items.PHANTOM_MEMBRANE) != -1 && hasBadEffect && !mc.player.getItemCooldownManager().isCoolingDown(Items.PHANTOM_MEMBRANE.getDefaultStack()) && this.auraCounter.a(5000L) && mc.player.getAbsorptionAmount() <= 3.0f) {
                    Laura.getInstance().getModuleProcessor().v().getUseableHandler().a(Items.PHANTOM_MEMBRANE.getDefaultStack());
                    this.auraCounter.b();
                }
            }
        }
        ...
        if (handledScreenAccessorB instanceof GenericContainerScreen) {
            ...
            if (handledScreenAccessor.getScreenHandler().slots.size() >= 90) {
                ...
                if (event.h() == ContainerEvent.Phase.POST && this.helpElements.a("Аукционный ассистент").c().booleanValue()) {
                    ...
                    Laura.getInstance().getModuleProcessor().i().a(event.getContext(), accessor.getX() + this.cheapestSlot.x, accessor.getY() + this.cheapestSlot.y, 16.0f, 16.0f, ColorUtil.convertToARGB(0, 255, 0, (int) (25.0f + (175.0f * pulse))));
                }
            }
        }
    }
    ...
    private BindSetting a(String name, Item item, String... servers) {
        BindSetting setting = new BindSetting(name, -1).a(() -> {
            ...
            if (InventoryUtil.b(item) == -1) {
                ...
                Laura.getInstance().getModuleProcessor().v().getUseableHandler().a(item.getDefaultStack());
            }
        }).a(() -> {
