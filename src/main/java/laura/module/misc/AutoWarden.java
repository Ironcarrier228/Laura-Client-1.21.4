...
import laura.module.render.WardenESP;
...
public class AutoWarden extends Module {
    ...
    public void b() { // onEnable
        ...
        if (!Laura.getInstance().getModuleProcessor().t().i().m()) {
            Laura.getInstance().getModuleProcessor().t().i().a();
        }
    ...
    private boolean isStuck() {
        ...
        return !isWardenAggro() && this.state == State.COLLECTING && isInFarmArea() && mc.currentScreen == null && !isMoving() && !Laura.getInstance().getModuleProcessor().v().getInteractHandler().hasTasks() && isInsideBlock();
    }
    ...
    private boolean isNearChest(double range) {
        for (BlockPos chest : Laura.getInstance().getModuleProcessor().t().i().scanChests()) {
    ...
    private void handleCollecting() {
        ...
        Laura.getInstance().getModuleProcessor().t().aV().b(18);
        if (Laura.getInstance().getModuleProcessor().v().getInteractHandler().hasTasks()) {
            if (isMoving()) {
                cancelPathing();
                return;
            }
    ...
    private void collect() {
        ...
        if (!isInFarmArea()) {
            ...
            if (speedSlot < 0) {
                ...
                Laura.getInstance().getModuleProcessor().v().getInteractHandler().addTask(speedSlot);
            }
        }
    }
    ...
    private void handleEscape() {
        ...
        if ((mc.currentScreen instanceof GenericContainerScreen) || (near != null && Laura.getInstance().getModuleProcessor().t().i().getRemainingTime(near) < 0 && mc.player.getEyePos().squaredDistanceTo(Vec3d.ofCenter(near)) <= 16.0d)) {
            handleChest();
            return;
        }
        if (ServerUtil.e()) {
    ...
    private void debugOutput() {
        ...
        ChatUtil.sendMessage("&7[AW] состояние &f" + this.state + " &7| анархия &f" + ServerUtil.a.d() + "&7, нужна &f" + currentAnarchy() + " &7(список: &f" + this.anarchyList.size() + "&7) | в зоне фермы &f" + isInFarmArea() + " &7| сундуков у ESP &f" + Laura.getInstance().getModuleProcessor().t().i().scanChests().size() + " &7| варден &f" + isWardenAggro() + " &7| пвп &f" + ServerUtil.e() + " &7| baritone: &f" + (pathing.hasPath() ? "идёт" : "стоит"));
    ...
    private void handleChest() {
        ...
        boolean stay = pick != null && this.targetChest != null && !pick.equals(this.targetChest) && Laura.getInstance().getModuleProcessor().t().i().getRemainingTime(this.targetChest) > 25000;
        if (!stay) {
            this.counter.b();
        }
        ...
        long remaining = Laura.getInstance().getModuleProcessor().t().i().getRemainingTime(target);
    ...
    private BlockPos findBestChest() {
        WardenESP esp = Laura.getInstance().getModuleProcessor().t().i();
        BlockPos best = null;
        long bestMs = 45000;
    ...
    private void drinkInvisibilityPotion() {
        ...
        if (slot >= 0 && mc.player.age > 20) {
            Laura.getInstance().getModuleProcessor().v().getInteractHandler().addTask(slot);
        }
    }
    ...
    private boolean interactChest(BlockPos chest, int rate) {
        ...
        Laura.getInstance().getModuleProcessor().k().startAiming(new Rotation(target.c() + sw, MathUtil.b(target.d() + (sw / 4.0f), -90.0f, 90.0f)), 120.0f, 1, 1);
        ...
        for (int i2 = 9; i2 < 36; i2++) {
            if (mc.player.getInventory().getStack(i2).isEmpty()) {
                ...
                if (mc.player.age % 10 == 4) {
                    Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(mc.player.getInventory().selectedSlot, i2, 1);
                    return;
                }
                return;
            }
