...
public class ElytraHelper extends Module {
    ...
    public ElytraHelper() {
        ...
        this.f = new BindSetting("Кнопка переключения", -1).a(() -> {
            ...
            Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveToArmor(slot, 1, 1);
    ...
    private void z() {
        if (mc.player.isGliding()) {
            Laura.getInstance().getModuleProcessor().v().getUseableHandler().a(Items.FIREWORK_ROCKET.getDefaultStack());
        }
    }
