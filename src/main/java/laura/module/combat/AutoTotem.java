...
public class AutoTotem extends Module {
    ...
    private boolean q() {
        ...
        Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(slot, 40, 1);
        return true;
    }
    ...
    private boolean r() {
        ...
        if (this.d.a("Возвращать предмет").c().booleanValue() && this.h != -1 && (offhand.isEmpty() || a(offhand))) {
            Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(this.h, 40, 1);
        }
        this.h = -1;
        return false;
    }
