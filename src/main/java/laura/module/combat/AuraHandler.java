...
public class AuraHandler extends BaseHandler implements Interface {
    ...
    public void onDraw(DrawEvent event) {
        ...
        if (event.c() && this.target != null) {
            float anim = this.e.c();
            if (anim > 0.0f) {
                int themeColor = Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.PRIMARY).toIntColor();
                ...
                enableRenderState();
                if (Laura.getInstance().getModuleProcessor().t().B().getVisualizationMode().l("Круг")) {
    ...
    public void onGlobalEvent(GlobalEvent event) {
        if (mc.player != null) {
            Laura.getInstance().getModuleProcessor().t().B().attackCooldown++;
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        LivingEntity current = aura.getTarget() != null ? aura.getTarget() : Laura.getInstance().getModuleProcessor().t().X().getTarget();
    ...
    private void drawBloom(MatrixStack stack, Vec3d renderPos, float ringWidth, float ringScale, float moving, int color, float anim) {
        ...
        int blue = rgba[2];
        int alpha = rgba[3];
        RenderSystem.setShaderTexture(0, Identifier.of("laura", "pictures/bloom.png"));
