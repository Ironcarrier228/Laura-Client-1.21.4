...
import laura.event.WillLandEvent;
...
public class Aura extends Module {
    ...
    private void restoreSelectedSlot() {
        ...
        if (this.savedSlots[1] != -1
                && Laura.getInstance().getModuleProcessor().v().getInventoryHandler().a().isEmpty()) {
            Laura.getInstance().getModuleProcessor().v().getInventoryHandler()
    ...
    private void performAttack() {
        ...
        if (this.shieldBreaking.c().booleanValue() && this.target.isBlocking()) {
            ...
            if (hotbar != -1) {
                ...
                if (inventory != -1 && this.savedSlots[1] == -1
                    ...
                    Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(inventory,
        ...
        if (canAttack()) {
            boolean skip = false;
            if ((Laura.getInstance().getModuleProcessor().t().H().e || (mc.player.fallDistance > 2.0f
                    && Laura.getInstance().getModuleProcessor().t().H().c.c().booleanValue()))
                ...
                if ((!this.willLand && !MaceUtil.b()
                        && Laura.getInstance().getModuleProcessor().t().H().b.c().booleanValue()
    ...
    private boolean isEntityReachable(LivingEntity entity) {
        if (Laura.getInstance().getModuleProcessor().t().G().m() && mc.player.isGliding()) {
            return true;
        }
        return AuraUtil.a((Entity) entity, ((double) (this.attackDistance.c().floatValue()
                ...
                + ((double) ((InventoryUtil.b(Items.MACE) == -1 || ((double) mc.player.fallDistance) <= 1.5d) ? 0.0f
                        : 1.5f))
                + ((double) ((Laura.getInstance().getModuleProcessor().t().H().m() && InventoryUtil.b(Items.MACE) != -1
    ...
    private boolean isValidTarget(LivingEntity entity) {
        ...
        if (entity instanceof PlayerEntity) {
            boolean isFriend = Laura.getInstance().getModuleProcessor().e().d(entity.getName().getString());
    ...
    private void rotateToTarget() {
        ...
        if (!this.rotationType.c().contains("ФанТайм")
                && (InventoryUtil.b(Items.MACE) != -1 || (Laura.getInstance().getModuleProcessor().t().H().e
    ...
    private void applyLegitSmoothing(float yawToTarget, float pitchToTarget, Vec3d vec3d) {
        ...
        Laura.getInstance().getModuleProcessor().k().startAiming(new Rotation(finalYaw + smoothW, finalPitch + smoothH),
                180.0f,
                1, 1);
    }
}
