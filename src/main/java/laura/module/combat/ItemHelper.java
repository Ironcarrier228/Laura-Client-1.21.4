...
public class ItemHelper extends Module {
    ...
    final class a {
        ...
        private void f() {
            if (this.d == -1) {
                ...
                if (from != -1 && from != target) {
                    this.d = from;
                    this.c = target;
                    Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(from, target, 1);
                }
            }
        }
        ...
        private void g() {
            if (this.d != -1) {
                Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(this.d, this.c, 1);
                this.d = -1;
                this.c = -1;
            }
        }
