...
public class AutoEXP extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (!this.isActive && this.d[0] != -1) {
            ...
            if (this.d[1] != -1) {
                Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(7, this.d[1], 1);
            }
            this.d[0] = -1;
            this.d[1] = -1;
        }
    ...
    private void throwExpBottles() {
        ...
        if (!inHand) {
            ...
            if (hotbarSlot != -1) {
                ...
                if (Laura.getInstance().getModuleProcessor().v().getInventoryHandler().a().isEmpty() && this.isActive && (invSlot = InventoryUtil.b(Items.EXPERIENCE_BOTTLE)) != -1) {
                    if (this.d[1] == -1) {
                        this.d[1] = invSlot;
                    }
                    ...
                    Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItem(invSlot, 7, 1);
                    return;
                }
                return;
            }
        ...
        Laura.getInstance().getModuleProcessor().k().startAiming(new Rotation(Look.b() + smoothYaw, 86.0f + smoothPitch), 70.0f, 1, 3);
