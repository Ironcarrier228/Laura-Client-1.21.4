...
import laura.command.BlockESPCommand;
...
public class BlockESP extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        this.renderTick = i;
        if (i % 12 == 0) {
            List<BlockESPCommand.a> list = Laura.getInstance().getModuleProcessor().u().g().c();
            if (list.isEmpty()) {
                this.b.clear();
            } else {
    ...
    public void onDraw(DrawEvent event) {
        if (event.c()) {
            List<BlockESPCommand.a> entries = Laura.getInstance().getModuleProcessor().u().g().c();
            if (!entries.isEmpty()) {
    ...
    private void renderBlock(DrawEvent event, BlockPos pos, Map<Block, Integer> colors) {
        ...
        if (color != null) {
            event.getDraw3DProcessor().a(event.h(), new Box(pos), color.intValue() != -1 ? color.intValue()
                            : ColorUtil.combineColorWithAlpha(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.PRIMARY).toIntColor(),
