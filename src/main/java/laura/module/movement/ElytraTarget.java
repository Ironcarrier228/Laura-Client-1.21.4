...
import laura.module.combat.Aura;
import laura.module.combat.AuraUtil;
...
public class ElytraTarget extends Module {
    ...
    public void a(TickEvent event) {
        ...
        if (aura.m() && mc.player.isGliding()) {
            if (mc.player.getOffHandStack().getItem() != Items.FIREWORK_ROCKET && !Laura.getInstance().getModuleProcessor().t().V().b) {
                if (Laura.getInstance().getModuleProcessor().v().getInventoryHandler().a().isEmpty()) {
                    Laura.getInstance().getModuleProcessor().v().getInventoryHandler().moveItemByType(Items.FIREWORK_ROCKET, 45, 1);
                }
            ...
            Laura.getInstance().getModuleProcessor().k().startAiming(new Rotation(Yaw, Pitch), 180.0f, 1, 1);
        }
    }
