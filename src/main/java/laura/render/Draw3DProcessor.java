...
public class Draw3DProcessor extends BaseProcessor {
    ...
    public void a(DrawEvent event) {
        if (event.c()) {
            Laura.getInstance().getModuleProcessor().l().b();
        } else if (event.b()) {
            Laura.getInstance().getModuleProcessor().l().a();
        }
    }

    public void a(MatrixStack matrices, Box box, int color, float width) {
        Laura.getInstance().getModuleProcessor().l().a(BatchProcessor.b.a(matrices, box, color, width));
    }
    ...
    public void a(Matrix4f matrix, float minX, float minY, float maxX, float maxY, int color, boolean corners, boolean healthBar, float healthPercent, int healthColor) {
        Laura.getInstance().getModuleProcessor().l().a(new BatchProcessor.a(matrix, minX, minY, maxX, maxY, color, corners, healthBar, healthPercent, healthColor));
    }

    public void a(MatrixStack matrices, Vec3d start, Vec3d end, Vec3d control, int color, float width) {
        Laura.getInstance().getModuleProcessor().l().a(BatchProcessor.b.a(matrices, start, end, control, color, width));
    }
