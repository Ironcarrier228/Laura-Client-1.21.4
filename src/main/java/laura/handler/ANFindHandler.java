    ...
    public void onContainerEvent(ContainerEvent event) {
        ...
        if (event.h() == ContainerEvent.Phase.POST) {
            if (event.getScreen().getTitle().getString().contains("☬ Выберите режим:") || event.getScreen().getTitle().getString().contains("☬ Выберите тип режима:")) {
                ...
                Laura.getInstance().getModuleProcessor().i().a(event.getContext().getMatrices(), Identifier.of("laura", this.isHovering ? "pictures/minecraft/join_button_hovered.png" : "pictures/minecraft/join_button.png"), x, y, 10.0f, 10.0f, 0.0f, -1);
                if (this.isHovering) {
