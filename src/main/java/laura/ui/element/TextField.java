...
public class TextField {
    ...
    public void render(DrawContext context, double mouseX, double mouseY, float delta, float alpha) {
        ...
        int color = ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(placeholding ? ThemeInfo.TEXT_DISABLED : ThemeInfo.TEXT).toIntColor(), alpha);
    ...
    private void drawSelectionHighlight(DrawContext context, float textX, float textY, float lineHeight, float visibleWidth, float alpha) {
        if (this.focused && this.selectionRange.getX() != this.selectionRange.getY()) {
            ...
            if (start < end) {
                Draw2DProcessor draw = Laura.getInstance().getModuleProcessor().i();
                draw.a(context, start, textY, end - start, lineHeight, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.PRIMARY).toIntColor(), 0.47f * alpha));
            }
        }
    }
    ...
    private void drawCaret(DrawContext context, float textX, float textY, float lineHeight, float visibleWidth, float alpha) {
        if (this.focused) {
            ...
            if (caretX >= textX && caretX <= textX + visibleWidth) {
                ...
                Draw2DProcessor draw = Laura.getInstance().getModuleProcessor().i();
                draw.a(context, caretX, caretY, 0.5f, caretHeight, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), blink * alpha));
            }
        }
    }
    ...
    public void a(char chr, int modifiers) {
        if (this.focused) {
            if (!this.numbersOnly || (chr >= '0' && chr <= '9')) {
