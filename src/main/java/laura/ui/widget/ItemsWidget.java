...
import laura.module.misc.ServerAssistant;
...
public class ItemsWidget extends Widget implements Interface {
    ...
    public void a(DrawEvent event) {
        ...
        ServerAssistant assistant = Laura.getInstance().getModuleProcessor().t().aj();
        ...
        while (i < providers.size()) {
            ...
            if (animation > 0.0f) {
                ...
                Laura.getInstance().getModuleProcessor().j().a(event.i(), provider2.c().getDefaultStack(), contentX + 3.0f, y + ((this.d - 16.0f) / 2.0f) + 3.0f, InterfaceC0020Opcode.aN, animation, 0.6f, false);
                a(event, contentX + 15.5f, y, this.d, animation);
                Fonts.e.a(event.h(), label, contentX + 19.5f, textY, 6.5f, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), animation));
