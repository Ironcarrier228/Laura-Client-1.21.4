    ...
    private void c(DrawEvent event) {
        ...
        while (it2.hasNext()) {
            ...
            if (animation > 0.0f) {
                ...
                Fonts.e.a(event.h(), name2, x + offsetX + 19.0f, textY, 6.5f, ColorUtil.applyAlphaToColor(iStatusEffectInstance2.getEffectType().value().getCategory() == StatusEffectCategory.HARMFUL ? ColorUtil.convertToARGB(255, InterfaceC0020Opcode.cG, InterfaceC0020Opcode.cG, 255) : Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), animation));
                Fonts.e.a(event.h(), duration, ((((x + offsetX) + width) - 5.0f) - durationWidth) - 1.0f, textY, 6.5f, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), 0.55f * animation));
    ...
    private void d(DrawEvent event) {
        ...
        while (it2.hasNext()) {
            ...
            if (animation > 0.0f) {
                ...
                Fonts.e.a(event.h(), name, textX, contentY + 3.5f, 7.0f, ColorUtil.applyAlphaToColor(harmful ? ColorUtil.convertToARGB(215, 76, 76, 255) : Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), animation));
                Fonts.e.a(event.h(), duration, textX, contentY + 13.0f, 6.0f, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), 0.55f * animation));
        ...
        for (StatusEffectInstance effect : k()) {
            if (!effect.getEffectType().equals(StatusEffects.NIGHT_VISION)) {
                ...
                if (effect != this.g && effect.getDuration() == 100 && (effect.getEffectType().equals(StatusEffects.STRENGTH) || effect.getEffectType().equals(StatusEffects.SPEED) || effect.getEffectType().equals(StatusEffects.HEALTH_BOOST) || effect.getEffectType().equals(StatusEffects.INVISIBILITY))) {
                    Laura.getInstance().getModuleProcessor().m().a(new Notification("E", Text.literal("Эффект ").append(Text.translatable(effect.getEffectType().value().getTranslationKey()).append(" " + (effect.getAmplifier() + 1)).styled(style -> {
                        return style.withColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.PRIMARY).toIntColor());
