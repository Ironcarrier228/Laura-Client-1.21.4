...
public class EnvironmentWidget extends Widget implements Interface {
    ...
    private float a(DrawEvent event, b data, float x, float y, float width, float animation) {
        ...
        Fonts.e.a(event.h(), health, ((x + width) - 3.0f) - Fonts.e.a(health, 6.5f), textY, 6.5f,
                ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), animation));
        ...
        Fonts.e.c(event.h(), data.d, x + 12.5f, textY, 6.5f, ColorUtil.applyAlphaToColor(Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.TEXT).toIntColor(), animation),
                (right - 16.5f) - x);
        if (data.b.isEmpty()) {
            return 14.0f;
        }
    ...
    public void a(GlobalEvent event) {
        ...
        if (mc.world != null && mc.player != null) {
            PlayerEntity class_1657VarS = (PlayerEntity) Laura.getInstance().getModuleProcessor().t().B().getTarget();
            if (class_1657VarS instanceof PlayerEntity) {
                PlayerEntity aura = class_1657VarS;
                class_1657Var = aura;
            } else {
                PlayerEntity class_1657VarS2 = (PlayerEntity) Laura.getInstance().getModuleProcessor().t().X().getTarget();
            ...
            List<? extends PlayerEntity> nearby = mc.world.getPlayers().stream().filter(player -> {
                return player != mc.player && player.isAlive()
                        && !Laura.getInstance().getModuleProcessor().e().d(player.getName().getString());
    ...
    static final class a {
        ...
        a(ItemStack stack, int hand) {
            ...
            this.a.set(DataComponentTypes.USE_COOLDOWN,
                    new UseCooldownComponent(0.0f, Optional.of(Identifier.of("laura", "widget"))));
            this.d = hand;
        }
    ...
    static final class b {
        ...
        void a(PlayerEntity player, long now) {
            ...
            StreamerMode streamer = Laura.getInstance().getModuleProcessor().t().aE();
