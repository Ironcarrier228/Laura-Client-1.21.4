...
public class NotificationWidget extends Widget implements Interface {
    ...
    public void a(GlobalEvent event) {
        d().a((mc.currentScreen instanceof ChatScreen) || !Laura.getInstance().getModuleProcessor().m().b().isEmpty());
        super.a(event);
    }
    ...
    public void a(DrawEvent event) {
        ...
        for (Notification notification : Laura.getInstance().getModuleProcessor().m().b()) {
            ...
            if (animation > 0.0f) {
                ...
                int color = notification.e() == -1 ? Laura.getInstance().getModuleProcessor().o().a(ThemeInfo.PRIMARY).toIntColor() : notification.e();
                ...
                if (class_746VarMethod_8469 instanceof PlayerEntity) {
                    ...
                    if (class_1542VarMethod_8469 instanceof ItemEntity) {
                        ...
                        if (class_746Var != mc.player && !itemEntity.getStack().getName().getString().contains("Упс.") && ((itemEntity.getStack().contains(DataComponentTypes.CUSTOM_NAME) && itemEntity.getStack().contains(DataComponentTypes.LORE)) || itemEntity.getStack().isOf(Items.ENCHANTED_GOLDEN_APPLE))) {
                            Laura.getInstance().getModuleProcessor().m().a(new Notification(itemEntity.getStack().copy(), class_746Var.getName().copy().append(" подобрал ").append(itemEntity.getStack().getName()).append(itemPickupAnimationS2CPacket.getStackAmount() > 1 ? " x" + itemPickupAnimationS2CPacket.getStackAmount() : ""), 1500));
                        }
                    }
                }
            }
    ...
    public void a(BackendEvent event) {
        ...
        if ("friend".equals(packet.getId()) && this.g.c().booleanValue() && message != null) {
            Laura.getInstance().getModuleProcessor().m().a(new Notification("o", message, 5000));
        }
        if ("application".equals(packet.getId()) && message != null) {
            Laura.getInstance().getModuleProcessor().m().a(new Notification("o", message, 15000));
        }
        super.a(event);
    }
}
