...
import laura.lib.websocket.ServerHandshake;
import laura.lib.websocket.WebSocketClient;
import laura.network.PacketSecurity;
...
public class Client extends WebSocketClient {
    ...
    public Client(boolean dev) {
        super(URI.create(dev ? "ws://localhost:2002/" : "wss://deltaclient.xyz/ws/"),
                Map.of("Sec-WebSocket-Protocol", Laura.getInstance().g().token() + "-minecraft"));
    ...
    public void c(String message) {
        try {
            Optional<laura.network.PacketSecurity.PacketData> unpacked = this.packetSecurity.unpackPacket(message);
            if (unpacked.isEmpty()) {
                return;
            }
            laura.network.PacketSecurity.PacketData data = unpacked.get();
            Packet packet = new Packet(data.id(), data.payload(), this.packetSecurity);
            laura.core.EventManager
                    .a(new laura.event.BackendEvent(packet, laura.event.BackendEvent.Phase.RECEIVE));
        } catch (Exception ex) {
            if (logger != null) {
    ...
    public void b(int code, String reason, boolean remote) {
        ...
        laura.core.EventManager.a(new laura.event.BackendEvent(laura.event.BackendEvent.Phase.CLOSE));
    }
